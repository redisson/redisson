...
//        return createRedisWithVersion("valkey/valkey-bundle:latest", params);
    ...
    protected static void testInClusterFactory(Consumer<RedissonConnectionFactory> redissonCallback) {
        if (redissonCluster == null) {
        ...
        redissonCallback.accept(new RedissonConnectionFactory(redissonCluster));
    }

    protected static void testInCluster(Consumer<RedissonClusterConnection> redissonCallback) {
        if (redissonCluster == null) {
            ClusterData data = createCluster();
            REDIS_CLUSTER = data.container;
            redissonCluster = data.redisson;
