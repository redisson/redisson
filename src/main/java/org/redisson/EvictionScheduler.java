...
/**
 * Eviction scheduler for RMapCache object. Deletes expired entries in time interval between 5
 * seconds to 2 hours. It analyzes deleted amount of expired keys and 'tune' next execution delay
 * depending on it.
 *
...
public class EvictionScheduler {
  ...
  public class RedissonCacheTask implements Runnable {
    ...
    public void run() {
      Future<Integer> future =
          cleanupExpiredEntires(name, timeoutSetName, maxIdleSetName, keysLimit);
  ...
  private final ConcurrentMap<String, RedissonCacheTask> tasks = PlatformDependent
      .newConcurrentHashMap();
  private final CommandAsyncExecutor executor;

  private final ConcurrentMap<String, Long> lastExpiredTime = PlatformDependent
      .newConcurrentHashMap();
  ...
  private Future<Integer> cleanupExpiredEntires(String name, String timeoutSetName,
      String maxIdleSetName, int keysLimit) {
    if (maxIdleSetName != null) {
      return executor
          .evalWriteAsync(
              ...
              Arrays.<Object>asList(name, timeoutSetName, maxIdleSetName),
    ...
    return executor.evalWriteAsync(name, LongCodec.INSTANCE, RedisCommands.EVAL_INTEGER,
        "local expiredKeys = redis.call('zrangebyscore', KEYS[2], 0, ARGV[1], 'limit', 0, ARGV[2]); "
            ...
            + "redis.call('hdel', KEYS[1], unpack(expiredKeys)); " + "end; "
            + "return #expiredKeys;", Arrays.<Object>asList(name, timeoutSetName),
        System.currentTimeMillis(), keysLimit);
  }
