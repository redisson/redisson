...
public interface RExpirableReactive extends RObjectReactive {
  ...
  /**
   * Set a timeout for object in mode. After the timeout has expired, the key will automatically be
   * deleted.
   *
  ...
  /**
   * Set an expire date for object in mode. When expire date comes the key will automatically be
   * deleted.
   *
   ...
   *
   * @return time in milliseconds -2 if the key does not exist. -1 if the key exists but has no
   *         associated expire.
   */
