...
public class RedissonSubList<V> extends RedissonList<V> implements RList<V> {
  ...
  public static final RedisCommand<Boolean> EVAL_BOOLEAN_ARGS2 = new RedisCommand<Boolean>("EVAL",
      new BooleanReplayConvertor(), 5, ValueType.OBJECTS);

  final int fromIndex;
  AtomicInteger toIndex = new AtomicInteger();
  int size = -1;

  protected RedissonSubList(Codec codec, CommandAsyncExecutor commandExecutor, String name,
  ...
  public Future<Integer> sizeAsync() {
    ...
    return commandExecutor.readAsync(getName(), codec, new RedisStrictCommand<Integer>("LLEN",
  ...
  public Future<List<V>> readAllAsync() {
    return commandExecutor.readAsync(getName(), codec, LRANGE, getName(), fromIndex,
        toIndex.get() - 1);
  }
  ...
  public Future<Boolean> containsAllAsync(Collection<?> c) {
    ...
    return commandExecutor
        .evalReadAsync(
            getName(),
            codec,
            new RedisCommand<Boolean>("EVAL", new BooleanReplayConvertor(), 6, ValueType.OBJECTS),
            "local fromIndex = table.remove(ARGV, 1);"
                ...
                + "for i=1, #items do " + "for j = 0, #ARGV, 1 do "
                + "if items[i] == ARGV[j] then " + "table.remove(ARGV, j) " + "end " + "end "
                + "end " + "return table.getn(ARGV) == 0 and 1 or 0", Collections
                .<Object>singletonList(getName()), params.toArray());
  }
  ...
  public Future<Boolean> addAllAsync(int index, Collection<? extends V> coll) {
    ...
    return commandExecutor.evalWriteAsync(getName(), codec, EVAL_BOOLEAN_ARGS2,
        "local ind = table.remove(ARGV, 1); "
            ...
            + "redis.call('ltrim', KEYS[1], 0, ind - 1); " + "for i=1, #ARGV, 5000 do "
            + "redis.call('rpush', KEYS[1], unpack(ARGV, i, math.min(i+4999, #ARGV))); " + "end; "
            + "if #tail > 0 then " + "for i=1, #tail, 5000 do "
            + "redis.call('rpush', KEYS[1], unpack(tail, i, math.min(i+4999, #tail))); " + "end "
            + "end;" + "return 1;", Collections.<Object>singletonList(getName()), args.toArray());
  }
  ...
  private Future<Boolean> removeAllAsync(Collection<?> c, int count) {
    ...
    return commandExecutor.evalWriteAsync(getName(), codec, new RedisCommand<Boolean>("EVAL",
        new BooleanReplayConvertor(), 7, ValueType.OBJECTS), "local v = 0; "
        + "local fromIndex = table.remove(ARGV, 1);" + "local toIndex = table.remove(ARGV, 2);"
        + "local count = table.remove(ARGV, 3);"
        + "local items = redis.call('lrange', KEYS[1], fromIndex, toIndex); " +

        "for i=1, #items do " + "for j = 0, #ARGV, 1 do " + "if items[i] == ARGV[j] then "
        + "redis.call('lrem', KEYS[1], count, ARGV[i]); " + "v = 1; " + "end; " + "end; " + "end; "
        + "return v; ", Collections.<Object>singletonList(getName()), params.toArray());
  }
  ...
  public Future<Boolean> retainAllAsync(Collection<?> c) {
    ...
    return commandExecutor.evalWriteAsync(getName(), codec, RedisCommands.EVAL_BOOLEAN_WITH_VALUES,
        "local changed = 0 " + "local fromIndex = table.remove(ARGV, 1);"
            + "local toIndex = table.remove(ARGV, 2);"
            + "local items = redis.call('lrange', KEYS[1], fromIndex, toIndex) " + "local i = 1 "
            + "local s = table.getn(items) " + "while i <= s do " + "local element = items[i] "
            + "local isInAgrs = false " + "for j = 0, table.getn(ARGV), 1 do "
            + "if ARGV[j] == element then " + "isInAgrs = true " + "break " + "end " + "end "
            + "if isInAgrs == false then " + "redis.call('LREM', KEYS[1], 0, element) "
            + "changed = 1 " + "end " + "i = i + 1 " + "end " + "return changed ",
  ...
  public void clear() {
    ...
    get(commandExecutor.evalWriteAsync(getName(), codec, RedisCommands.EVAL_VOID,
        "local tail = redis.call('lrange', KEYS[1], ARGV[2], -1); "
            + "redis.call('ltrim', KEYS[1], 0, ARGV[1] - 1); " + "if #tail > 0 then "
            + "for i=1, #tail, 5000 do "
            + "redis.call('rpush', KEYS[1], unpack(tail, i, math.min(i+4999, #tail))); " + "end "
            + "end;", Collections.<Object>singletonList(getName()), fromIndex, toIndex));
    size = 0;
  }
  ...
  private void checkIndex(int index) {
    if (!(index >= fromIndex && index < toIndex.get())) {
      throw new IndexOutOfBoundsException("index: " + index + " but current fromIndex: "
  ...
  public Future<V> setAsync(int index, V element) {
    ...
    return commandExecutor.evalWriteAsync(getName(), codec, new RedisCommand<Object>("EVAL", 5),
        "local v = redis.call('lindex', KEYS[1], ARGV[1]); "
            + "redis.call('lset', KEYS[1], ARGV[1], ARGV[2]); " + "return v",
  ...
  public Future<Void> fastSetAsync(int index, V element) {
    checkIndex(index);
    return commandExecutor.writeAsync(getName(), codec, RedisCommands.LSET, getName(), index,
        element);
  }
  ...
  private V removeInner(int index) {
    ...
    Future<V> f =
        commandExecutor.evalWriteAsync(getName(), codec, EVAL_OBJECT,
            "local v = redis.call('lindex', KEYS[1], ARGV[1]); "
                + "local tail = redis.call('lrange', KEYS[1], ARGV[1] + 1, -1);"
                + "redis.call('ltrim', KEYS[1], 0, ARGV[1] - 1);" + "if #tail > 0 then "
                + "for i=1, #tail, 5000 do "
                + "redis.call('rpush', KEYS[1], unpack(tail, i, math.min(i+4999, #tail))); "
                + "end " + "end;" + "return v", Collections.<Object>singletonList(getName()),
            fromIndex + index);
    return get(f);
  }

  private <R> Future<R> indexOfAsync(Object o, Convertor<R> convertor) {
    return commandExecutor.evalReadAsync(getName(), codec,
        new RedisCommand<R>("EVAL", convertor, 4),
        "local items = redis.call('lrange', KEYS[1], tonumber(ARGV[2]), tonumber(ARGV[3])) "
            + "for i=1,#items do " + "if items[i] == ARGV[1] then "
            + "return tonumber(ARGV[2]) + i - 1; " + "end; " + "end; " + "return -1; ",
        Collections.<Object>singletonList(getName()), o, fromIndex, toIndex.get() - 1);
  }

  @Override
  public Future<Integer> lastIndexOfAsync(Object o) {
    return commandExecutor.evalReadAsync(getName(), codec, new RedisCommand<Integer>("EVAL",
        new IntegerReplayConvertor(), 4), "local key = KEYS[1] " + "local obj = ARGV[1] "
        + "local fromIndex = table.remove(ARGV, 1);" + "local toIndex = table.remove(ARGV, 2);"
        + "local items = redis.call('lrange', key, tonumber(fromIndex), tonumber(toIndexs)) "
        + "for i = #items, 0, -1 do " + "if items[i] == obj then "
        + "return tonumber(ARGV[2]) + i - 1 " + "end; " + "end; " + "return -1; ", Collections
        .<Object>singletonList(getName()), o, fromIndex, toIndex.get() - 1);
  }
