...
class RedissonSubSortedSet<V> implements SortedSet<V> {
  ...
  public RedissonSubSortedSet(RedissonSortedSet<V> redissonSortedSet,
  ...
  public <T> T[] toArray(T[] a) {
    ...
    // double headScore = getHeadScore(connection);
    // double tailScore = getTailScore(connection);
    // return connection.zrangebyscore(redissonSortedSet.getName(), headScore,
    // tailScore).toArray(a);
    // } finally {
    // connectionManager.releaseRead(-1, connection);
    // }
  ...
  public SortedSet<V> subSet(V fromElement, V toElement) {
    ...
    // toElement = tailValue;
    // }
    // return new RedissonSubSortedSet<V>(redissonSortedSet, connectionManager, fromElement,
    // toElement);
  }
  ...
  public V first() {
    ...
    // double tailScore = getTailScore(connection);
    // List<V> vals = connection.zrangebyscore(redissonSortedSet.getName(), headScore.getScore(),
    // tailScore);
    // if (vals.isEmpty()) {
  ...
  public V last() {
    ...
    // double headScore = getHeadScore(connection);
    // List<V> vals = connection.zrangebyscore(redissonSortedSet.getName(), headScore,
    // tailScore.getScore());
    // if (vals.isEmpty()) {
    // throw new NoSuchElementException();
    // }
