...
public class RedissonBitSet extends RedissonExpirable implements RBitSet {
  ...
  public Future<Void> setAsync(long bitIndex, boolean value) {
    return commandExecutor.writeAsync(getName(), codec, RedisCommands.SETBIT_VOID, getName(),
        bitIndex, value ? 1 : 0);
  }
  ...
  public Future<byte[]> toByteArrayAsync() {
    return commandExecutor.readAsync(getName(), ByteArrayCodec.INSTANCE, RedisCommands.GET,
        getName());
  }
  ...
  public Future<Void> setAsync(BitSet bs) {
    return commandExecutor.writeAsync(getName(), ByteArrayCodec.INSTANCE, RedisCommands.SET,
