...
public class CommandDecoder extends ReplayingDecoder<State> {
  ...
  private final Map<String, MultiDecoder<Object>> messageDecoders =
      new HashMap<String, MultiDecoder<Object>>();
  private final Map<String, CommandData<Object, Object>> channels = PlatformDependent
    ...
    if (i == commands.getCommands().size()) {
      ...
      if (error != null) {
        if (!promise.tryFailure(error) && promise.cause() instanceof RedisTimeoutException) {
          log.warn("response has been skipped due to timeout! channel: {}, command: {}",
              ctx.channel(), data);
        }
      } else {
  ...
  private void decode(ByteBuf in, CommandData<Object, Object> data, List<Object> parts,
      Channel channel, Decoder<Object> currentDecoder) throws IOException {
    ...
    if (code == '+') {
      ...
      if (error.startsWith("MOVED")) {
        ...
        data.getPromise().tryFailure(
            new RedisLoadingException(error + ". channel: " + channel + " data: " + data));
      } else {
        data.getPromise().tryFailure(
            new RedisException(error + ". channel: " + channel + " command: " + data));
      }
    } else if (code == ':') {
  ...
  private void handleResult(CommandData<Object, Object> data, List<Object> parts, Object result,
      boolean multiResult, Channel channel) {
    if (data != null) {
      if (multiResult) {
    ...
    if (parts != null) {
      ...
      if (!data.getPromise().trySuccess(result)
          && data.getPromise().cause() instanceof RedisTimeoutException) {
        log.warn("response has been skipped due to timeout! channel: {}, command: {}, result: {}",
            channel, data, result);
      }
    }
  }
  ...
  private MultiDecoder<Object> messageDecoder(CommandData<Object, Object> data, List<Object> parts,
      Channel channel) {
    if (data == null) {
      if (Arrays.asList("subscribe", "psubscribe", "punsubscribe", "unsubscribe").contains(
          parts.get(0))) {
  ...
  private Decoder<Object> decoder(CommandData<Object, Object> data, List<Object> parts,
      Decoder<Object> currentDecoder) {
    if (data == null) {
  ...
  public ByteBuf readBytes(ByteBuf is) throws IOException {
    ...
    if (l > Integer.MAX_VALUE) {
      throw new IllegalArgumentException("Java only supports arrays up to " + Integer.MAX_VALUE
          + " in size");
    }
    int size = (int) l;
    if (size == -1) {
