 ...
 *
 */
public class RedissonAtomicLongReactive extends RedissonExpirableReactive implements
    RAtomicLongReactive {
  ...
  public Publisher<Long> addAndGet(long delta) {
    return commandExecutor.writeReactive(getName(), StringCodec.INSTANCE, RedisCommands.INCRBY,
        getName(), delta);
  }
  ...
  public Publisher<Boolean> compareAndSet(long expect, long update) {
    return commandExecutor.evalWriteReactive(getName(), StringCodec.INSTANCE,
        RedisCommands.EVAL_BOOLEAN, "if redis.call('get', KEYS[1]) == ARGV[1] then "
  ...
  public Publisher<Long> decrementAndGet() {
    return commandExecutor.writeReactive(getName(), StringCodec.INSTANCE, RedisCommands.DECR,
        getName());
  }
  ...
  public Publisher<Long> getAndAdd(final long delta) {
    return commandExecutor.writeReactive(getName(), StringCodec.INSTANCE,
        new RedisStrictCommand<Long>("INCRBY", new SingleConvertor<Long>() {
          @Override
  ...
  public Publisher<Long> getAndSet(long newValue) {
    return commandExecutor.writeReactive(getName(), LongCodec.INSTANCE, RedisCommands.GETSET,
        getName(), newValue);
  }

  @Override
  public Publisher<Long> incrementAndGet() {
    return commandExecutor.writeReactive(getName(), StringCodec.INSTANCE, RedisCommands.INCR,
        getName());
  }
  ...
  public Publisher<Void> set(long newValue) {
    return commandExecutor.writeReactive(getName(), StringCodec.INSTANCE, RedisCommands.SET,
        getName(), newValue);
  }
