 ...
 * {@link #putIfAbsent(Object, Object, long, TimeUnit)} method. And therefore has an complex
 * lua-scripts inside.
 * </p>
 *
 * <p>
 ...
 * @param <V> value
 */
public class RedissonMapCacheReactive<K, V> extends RedissonMapReactive<K, V> implements
  ...
  private static final RedisCommand<Object> EVAL_PUT_TTL = new RedisCommand<Object>("EVAL", 6,
      ValueType.MAP, ValueType.MAP_VALUE);
  private static final RedisCommand<List<Object>> EVAL_GET_TTL = new RedisCommand<List<Object>>(
      "EVAL", new TTLMapValueReplayDecoder<Object>(), 5, ValueType.MAP_KEY, ValueType.MAP_VALUE);
  private static final RedisCommand<List<Object>> EVAL_CONTAINS_KEY =
      new RedisCommand<List<Object>>("EVAL", new ObjectListReplayDecoder<Object>(), 5,
          ValueType.MAP_KEY);
  private static final RedisCommand<List<Object>> EVAL_CONTAINS_VALUE =
      new RedisCommand<List<Object>>("EVAL", new ObjectListReplayDecoder<Object>(), 5,
          ValueType.MAP_VALUE);
  ...
  public RedissonMapCacheReactive(EvictionScheduler evictionScheduler,
      CommandReactiveExecutor commandExecutor, String name) {
    super(commandExecutor, name);
    this.evictionScheduler = evictionScheduler;
    evictionScheduler.schedule(getName(), getTimeoutSetName());
  }

  public RedissonMapCacheReactive(Codec codec, EvictionScheduler evictionScheduler,
      CommandReactiveExecutor commandExecutor, String name) {
  ...
  public Publisher<Boolean> containsKey(Object key) {
    ...
    Publisher<List<Object>> future =
        commandExecutor.evalReadReactive(getName(), codec, EVAL_CONTAINS_KEY,
            "local value = redis.call('hexists', KEYS[1], ARGV[1]); "
                + "local expireDate = 92233720368547758; " + "if value == 1 then "
                + "local expireDateScore = redis.call('zscore', KEYS[2], ARGV[1]); "
                + "if expireDateScore ~= false then " + "expireDate = tonumber(expireDateScore) "
                + "end; " + "end;" + "return {expireDate, value}; ",
  ...
  public Publisher<Boolean> containsValue(Object value) {
    ...
    Publisher<List<Object>> future =
        commandExecutor.evalReadReactive(getName(), codec, EVAL_CONTAINS_VALUE,
            "local s = redis.call('hgetall', KEYS[1]);" + "for i, v in ipairs(s) do "
                + "if i % 2 == 0 and ARGV[1] == v then " + "local key = s[i-1];"
                + "local expireDate = redis.call('zscore', KEYS[2], key); "
                + "if expireDate == false then " + "expireDate = 92233720368547758 " + "else "
                + "expireDate = tonumber(expireDate) " + "end; " + "return {expireDate, 1}; "
                + "end " + "end;" + "return {92233720368547758, 0};",
  ...
  public Publisher<Map<K, V>> getAll(Set<K> keys) {
    ...
    Publisher<List<Object>> publisher =
        commandExecutor.evalReadReactive(getName(), codec, new RedisCommand<List<Object>>("EVAL",
            new CacheGetAllDecoder(args), 6, ValueType.MAP_KEY, ValueType.MAP_VALUE),
            "local expireHead = redis.call('zrange', KEYS[2], 0, 0, 'withscores');"
                ...
                + "ARGV[i] = ARGV[i] .. '__redisson__skip' " + "end;" + "end;" + "end; "
  ...
  public Publisher<V> putIfAbsent(K key, V value, long ttl, TimeUnit unit) {
    ...
    return commandExecutor.evalWriteReactive(getName(), codec, EVAL_PUT_TTL,
        "if redis.call('hexists', KEYS[1], ARGV[2]) == 0 then "
            ...
            + "redis.call('hset', KEYS[1], ARGV[2], ARGV[3]); " + "return nil " + "else "
            + "return redis.call('hget', KEYS[1], ARGV[2]) " + "end",
  ...
  public Publisher<V> get(K key) {
    ...
    Publisher<List<Object>> future =
        commandExecutor.evalReadReactive(getName(), codec, EVAL_GET_TTL,
            "local value = redis.call('hget', KEYS[1], ARGV[1]); "
                + "local expireDate = redis.call('zscore', KEYS[2], ARGV[1]); "
                + "if expireDate == false then " + "expireDate = 92233720368547758; " + "end; "
  ...
  private <T> void addExpireListener(final Promise<T> result, Publisher<List<Object>> publisher,
      final Convertor<T> convertor, final T nullValue) {
    publisher.subscribe(new DefaultSubscriber<List<Object>>() {
  ...
  public Publisher<V> put(K key, V value, long ttl, TimeUnit unit) {
    ...
    return commandExecutor.evalWriteReactive(getName(), codec, EVAL_PUT_TTL,
        "local v = redis.call('hget', KEYS[1], ARGV[2]); "
            + "redis.call('zadd', KEYS[2], ARGV[1], ARGV[2]); "
            + "redis.call('hset', KEYS[1], ARGV[2], ARGV[3]); " + "return v",
        Arrays.<Object>asList(getName(), getTimeoutSetName()), timeoutDate, key, value);
  }
  ...
  public Publisher<V> remove(K key) {
    return commandExecutor.evalWriteReactive(getName(), codec, EVAL_REMOVE,
        "local v = redis.call('hget', KEYS[1], ARGV[1]); "
            + "redis.call('zrem', KEYS[2], ARGV[1]); " + "redis.call('hdel', KEYS[1], ARGV[1]); "
            + "return v", Arrays.<Object>asList(getName(), getTimeoutSetName()), key);
  }
  ...
  public Publisher<Long> fastRemove(K... keys) {
    ...
    return commandExecutor.evalWriteReactive(getName(), codec, EVAL_FAST_REMOVE, "local r = 0;"
        + "for i=1, #ARGV,5000 do "
        + "r += redis.call('hdel', KEYS[1], unpack(ARGV, i, math.min(i+4999, #ARGV))); "
        + "redis.call('zrem', KEYS[2], unpack(ARGV, i, math.min(i+4999, #ARGV))); " + "end "
        + "return r;", Arrays.<Object>asList(getName(), getTimeoutSetName()), keys);
  }

  @Override
  Publisher<MapScanResult<ScanObjectEntry, ScanObjectEntry>> scanIteratorReactive(
    ...
    return commandExecutor
        .evalReadReactive(
            ...
            "local result = {}; "
                ...
                + "table.insert(result, key); " + "table.insert(result, value); " + "end; "
                + "end; " + "end;" + "return {res[1], result};",
            Arrays.<Object>asList(getName(), getTimeoutSetName()), startPos,
  ...
  public Publisher<Boolean> expire(long timeToLive, TimeUnit timeUnit) {
    return commandExecutor.evalWriteReactive(getName(), LongCodec.INSTANCE,
  ...
  public Publisher<Boolean> clearExpire() {
    return commandExecutor.evalWriteReactive(getName(), LongCodec.INSTANCE,
        RedisCommands.EVAL_BOOLEAN, "redis.call('zrem', KEYS[2], 'redisson__expiretag'); "
            + "redis.call('persist', KEYS[2]); " + "return redis.call('persist', KEYS[1]); ",
