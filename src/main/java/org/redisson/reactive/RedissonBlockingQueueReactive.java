 ...
 */
public class RedissonBlockingQueueReactive<V> extends RedissonQueueReactive<V> implements
  ...
  public RedissonBlockingQueueReactive(Codec codec, CommandReactiveExecutor commandExecutor,
      String name) {
  ...
  public Publisher<V> poll(long timeout, TimeUnit unit) {
    return commandExecutor.writeReactive(getName(), codec, RedisCommands.BLPOP_VALUE, getName(),
   ...
   * (non-Javadoc)
   * 
   * @see org.redisson.core.RBlockingQueueAsync#pollFromAnyAsync(long,
  ...
  public Publisher<V> pollFromAny(long timeout, TimeUnit unit, String... queueNames) {
    ...
    return commandExecutor.writeReactive(getName(), codec, RedisCommands.BLPOP_VALUE,
        params.toArray());
  }

  @Override
  public Publisher<V> pollLastAndOfferFirstTo(String queueName, long timeout, TimeUnit unit) {
    return commandExecutor.writeReactive(getName(), codec, RedisCommands.BRPOPLPUSH, getName(),
        queueName, unit.toSeconds(timeout));
  }
  ...
  public Publisher<Integer> drainTo(Collection<? super V> c) {
    ...
    return commandExecutor.evalWriteReactive(getName(), codec, new RedisCommand<Object>("EVAL",
        new ListDrainToDecoder(c)), "local vals = redis.call('lrange', KEYS[1], 0, -1); "
        + "redis.call('ltrim', KEYS[1], -1, 0); " + "return vals", Collections
        .<Object>singletonList(getName()));
  }
  ...
  public Publisher<Integer> drainTo(Collection<? super V> c, int maxElements) {
    ...
    return commandExecutor.evalWriteReactive(getName(), codec, new RedisCommand<Object>("EVAL",
        new ListDrainToDecoder(c)),
        "local elemNum = math.min(ARGV[1], redis.call('llen', KEYS[1])) - 1;"
            + "local vals = redis.call('lrange', KEYS[1], 0, elemNum); "
            + "redis.call('ltrim', KEYS[1], elemNum + 1, -1); " + "return vals", Collections
            .<Object>singletonList(getName()), maxElements);
  }
}
