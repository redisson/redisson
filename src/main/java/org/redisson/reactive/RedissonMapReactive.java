 ...
 * @param <V> value
 */
public class RedissonMapReactive<K, V> extends RedissonExpirableReactive implements
    RMapReactive<K, V> {
  ...
  Publisher<MapScanResult<ScanObjectEntry, ScanObjectEntry>> scanIteratorReactive(
    ...
    return commandExecutor.readReactive(client, getName(), new ScanCodec(codec),
        RedisCommands.HSCAN, getName(), startPos);
  }
  ...
  public boolean equals(Object o) {
    ...
    if (o instanceof Map) {
      ...
      return Streams.create(entryIterator()).map(mapFunction(m)).reduce(true, booleanAnd()).next()
          .poll();
  ...
  private Function<Entry<K, V>, Boolean> mapFunction(final RMapReactive<Object, Object> m) {
    return new Function<Map.Entry<K, V>, Boolean>() {
      ...
      public Boolean apply(Entry<K, V> e) {
        ...
        if (value == null) {
          if (!(Streams.create(m.get(key)).next().poll() == null && Streams
              .create(m.containsKey(key)).next().poll()))
            return false;
        } else {
