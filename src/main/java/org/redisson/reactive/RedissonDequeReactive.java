...
public class RedissonDequeReactive<V> extends RedissonQueueReactive<V> implements RDequeReactive<V> {
  ...
  private static final RedisCommand<Void> RPUSH_VOID = new RedisCommand<Void>("RPUSH",
      new VoidReplayConvertor(), 2, ValueType.OBJECTS);
  private static final RedisCommand<Object> LRANGE_SINGLE = new RedisCommand<Object>("LRANGE",
  ...
  public Publisher<Boolean> offerFirst(V e) {
    return commandExecutor.writeReactive(getName(), codec, RedisCommands.LPUSH_BOOLEAN, getName(),
        e);
  }
