...
public class CacheConfig {
  ...
  /**
   * Creates config object with <code>ttl = 0</code> and <code>maxIdleTime = 0</code>.
   *
   */
  public CacheConfig() {}
   ...
   *
   * @param ttl - time to live for key\value entry in milliseconds. If <code>0</code> then time to
   *        live doesn't affect entry expiration.
   * @param ttlUnit
   * @param maxIdleTime - max idle time for key\value entry in milliseconds. If <code>0</code> then
   *        max idle time doesn't affect entry expiration.
   * @param maxIdleUnit <p/>
