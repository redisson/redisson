 ...
 *
 */
public class RedissonSpringCacheManager implements CacheManager, ResourceLoaderAware,
    InitializingBean {
  ...
  /**
   * Creates CacheManager supplied by Redisson instance, Codec instance and Cache config mapped by
   * Cache name.
   * <p/>
   ...
   * Creates CacheManager supplied by Redisson instance and Cache config mapped by Cache name.
   * <p/>
   * Loads the config file from the class path, interpreting plain paths as class path resource
   * names that include the package path (e.g. "mypackage/myresource.txt").
   *
   * @param redisson
   * @param config
   */
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    try {
      ...
      try {
        ...
        throw new BeanDefinitionStoreException("Could not parse cache configuration at ["
            + configLocation + "]", e1);
      }
    }
  }
