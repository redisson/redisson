...
public class RedissonSortedSet<V> extends RedissonObject implements RSortedSet<V> {
  ...
  protected RedissonSortedSet(CommandExecutor commandExecutor, String name) {
    ...
    commandExecutor.write(getName(), StringCodec.INSTANCE, RedisCommands.SETNX,
        getCurrentVersionKey(), 0L);
  }
    ...
    try {
      ...
      if (comparatorSign != null) {
        ...
        if (!result.equals(sign)) {
          throw new IllegalStateException("Local class signature of " + className
    ...
    while (true) {
      ...
      if (res.getIndex() < 0) {
        ...
        if (index >= size()) {
          ...
          connection.sync(codec, RedisCommands.LINSERT, getName(), before ? "BEFORE" : "AFTER",
              pivot, value);
        }
  ...
  public boolean trySetComparator(Comparator<? super V> comparator) {
    ...
    Boolean res =
        commandExecutor.evalWrite(getName(), StringCodec.INSTANCE, RedisCommands.EVAL_BOOLEAN,
            "if redis.call('llen', KEYS[1]) == 0 then " + "redis.call('set', KEYS[2], ARGV[1]); "
   ...
   * @return
   */
  private BinarySearchResult<V> binarySearch(V value, Codec codec, RedisConnection connection,
