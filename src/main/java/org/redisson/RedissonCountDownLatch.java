 ...
 *
 * It has a advantage over {@link java.util.concurrent.CountDownLatch} -- count can be reset via
 * {@link #trySetCount}.
 *
...
public class RedissonCountDownLatch extends RedissonObject implements RCountDownLatch {
  ...
  public Future<Void> countDownAsync() {
    return commandExecutor.evalWriteAsync(getName(), LongCodec.INSTANCE,
        RedisCommands.EVAL_BOOLEAN, "local v = redis.call('decr', KEYS[1]);"
  ...
  public Future<Long> getCountAsync() {
    return commandExecutor.readAsync(getName(), LongCodec.INSTANCE, RedisCommands.GET_LONG,
        getName());
  }
  ...
  public Future<Boolean> trySetCountAsync(long count) {
    return commandExecutor.evalWriteAsync(getName(), LongCodec.INSTANCE,
        RedisCommands.EVAL_BOOLEAN, "if redis.call('exists', KEYS[1]) == 0 then "
            + "redis.call('set', KEYS[1], ARGV[2]); " + "redis.call('publish', KEYS[2], ARGV[1]); "
  ...
  public Future<Boolean> deleteAsync() {
    return commandExecutor.evalWriteAsync(getName(), LongCodec.INSTANCE,
        RedisCommands.EVAL_BOOLEAN, "if redis.call('del', KEYS[1]) == 1 then "
            ...
            + "end", Arrays.<Object>asList(getName(), getChannelName()), newCountMessage);
  }
