...
/**
 * Distributed implementation of {@link java.util.concurrent.locks.Lock} Implements reentrant lock.
...
public interface RLock extends Lock, RExpirable {
   ...
   *
   * If the lock is acquired, it is held until <code>unlock</code> is invoked, or until leaseTime
   * have passed since the lock was granted - whichever comes first.
   *
   * @param leaseTime the maximum time to hold the lock after granting it, before automatically
   *        releasing it if it hasn't already been released by invoking <code>unlock</code>. If
   *        leaseTime is -1, hold the lock until explicitly unlocked.
  ...
  /**
   * Returns <code>true</code> as soon as the lock is acquired. If the lock is currently held by
   * another thread in this or any other process in the distributed system this method keeps trying
   * to acquire the lock for up to <code>waitTime</code> before giving up and returning
   * <code>false</code>. If the lock is acquired, it is held until <code>unlock</code> is invoked,
   * or until <code>leaseTime</code> have passed since the lock was granted - whichever comes first.
   *
   ...
   *
   * @return @return <code>true</code> if held by current thread otherwise <code>false</code>
   */
