...
public interface RSemaphore extends RExpirable {
   ...
   * <ul>
   * <li>Some other thread invokes the {@link #release} method for this semaphore and the current
   ...
   *
   * <p>
   * Acquires the given number of permits, if they are available, and returns immediately, reducing
   ...
   * <ul>
   * <li>Some other thread invokes one of the {@link #release() release} methods for this semaphore,
   * the current thread is next to be assigned permits and the number of available permits satisfies
   * this request; or
   * <li>Some other thread {@linkplain Thread#interrupt interrupts} the current thread.
   * </ul>
   *
   ...
   *
   * <p>
   * Acquires a permits, if all are available and returns immediately, with the value {@code true},
   * reducing the number of available permits by given number of permitss.
   *
   * <p>
   * If no permits are available then this method will return immediately with the value
   * {@code false}.
   *
   * @param permits the number of permits to acquire
   * @return {@code true} if a permit was acquired and {@code false} otherwise
   ...
   *
   * <p>
   * If the specified waiting time elapses then the value {@code false} is returned. If the time is
   ...
   *
   * <p>
   * Releases a permit, increasing the number of available permits by one. If any threads of
   * Redisson client are trying to acquire a permit, then one is selected and given the permit that
   * was just released.
   *
   * <p>
   * There is no requirement that a thread that releases a permit must have acquired that permit by
   * calling {@link #acquire}. Correct usage of a semaphore is established by programming convention
   * in the application.
   */
  void release();
   ...
   *
   * <p>
   * Releases the given number of permits, increasing the number of available permits by the given
   * number of permits. If any threads of Redisson client are trying to acquire a permits, then next
   * threads is selected and tries to acquire the permits that was just released.
   *
   * <p>
   * There is no requirement that a thread that releases a permits must have acquired that permit by
   * calling {@link #acquire}. Correct usage of a semaphore is established by programming convention
   * in the application.
   */
   ...
   *
   * @param count - number of times {@link #countDown} must be invoked before threads can pass
   *        through {@link #await}
   */
