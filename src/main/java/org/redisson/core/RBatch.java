 ...
 * <p/>
 * All method invocations on objects from this interface are batched to separate queue and could be
 * executed later with <code>execute()</code> or <code>executeAsync()</code> methods.
 * <p/>
...
public interface RBatch {
  ...
  /**
   * Returns set-based cache instance by <code>name</code>. Uses map (value_hash, value) under the
   * hood for minimal memory consumption. Supports value eviction with a given TTL value.
   *
   * <p>
  ...
  /**
   * Returns set-based cache instance by <code>name</code> using provided <code>codec</code> for
   * values. Uses map (value_hash, value) under the hood for minimal memory consumption. Supports
   * value eviction with a given TTL value.
   *
   * <p>
   * If eviction is not required then it's better to use regular map {@link #getSet(String, Codec)}.
   * </p>
   ...
   * both cache keys and values. Supports entry eviction with a given TTL value.
   *
   * <p>
  ...
  /**
   * Returns map-based cache instance by <code>name</code>. Supports entry eviction with a given TTL
   * value.
   *
   * <p>
   ...
   *
   * If cluster configuration used then operations are grouped by slot ids and may be executed on
   * different servers. Thus command execution order could be changed
   *
   ...
   *
   * In cluster configurations operations grouped by slot ids so may be executed on different
   * servers. Thus command execution order could be changed
   *
   * @return List with result object for each command
   */
