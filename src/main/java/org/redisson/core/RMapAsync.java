...
public interface RMapAsync<K, V> extends RExpirableAsync {
   ...
   *
   * Works faster than <code>RMap.removeAsync</code> but doesn't return the value associated with
   * <code>key</code>
   *
   * @param keys
  ...
  /**
   * Associates the specified <code>value</code> with the specified <code>key</code> in async
   * manner.
   *
   * Works faster than <code>RMap.putAsync</code> but not returning the previous value associated
   * with <code>key</code>
   *
   * @param key
   * @param value
