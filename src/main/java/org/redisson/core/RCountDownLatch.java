 ...
 *
 * It has an advantage over {@link java.util.concurrent.CountDownLatch} -- count can be set via
 * {@link #trySetCount} method.
 *
...
public interface RCountDownLatch extends RObject, RCountDownLatchAsync {
   ...
   *        through {@link #await}
   * @return <code>true</code> if new count setted <code>false</code> if previous count has not
   *         reached zero
   */
