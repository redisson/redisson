...
public class ClientConnectionsEntry {
  ...
  private final Queue<RedisPubSubConnection> allSubscribeConnections =
      new ConcurrentLinkedQueue<RedisPubSubConnection>();
  ...
  private final Queue<RedisConnection> freeConnections =
      new ConcurrentLinkedQueue<RedisConnection>();
  ...
  public ClientConnectionsEntry(RedisClient client, int poolMinSize, int poolMaxSize,
      int subscribePoolMinSize, int subscribePoolMaxSize, ConnectionManager connectionManager,
      NodeType serverMode, MasterSlaveServersConfig config) {
    this.client = client;
    ...
    if (subscribePoolMaxSize > 0) {
      connectionManager.getConnectionWatcher().add(subscribePoolMinSize, subscribePoolMaxSize,
          freeSubscribeConnections, freeSubscribeConnectionsCounter);
    }
    connectionManager.getConnectionWatcher().add(poolMinSize, poolMaxSize, freeConnections,
  ...
  public Future<RedisConnection> connect(final MasterSlaveServersConfig config) {
    final Promise<RedisConnection> connectionFuture =
        client.getBootstrap().group().next().newPromise();
  ...
  private <T extends RedisConnection> void addReconnectListener(Promise<T> connectionFuture,
      final MasterSlaveServersConfig config, T conn) {
  ...
  private <T extends RedisConnection> void addFireEventListener(Promise<T> connectionFuture) {
    if (connectionFuture.isSuccess()) {
      connectionManager.getConnectionEventsHub().fireConnect(
          connectionFuture.getNow().getRedisClient().getAddr());
      return;
    }
    ...
    connectionFuture.addListener(new FutureListener<T>() {
      ...
      public void operationComplete(Future<T> future) throws Exception {
        if (future.isSuccess()) {
          connectionManager.getConnectionEventsHub().fireConnect(
              future.getNow().getRedisClient().getAddr());
        }
      }
    });
  }
  ...
  public Future<RedisPubSubConnection> connectPubSub(final MasterSlaveServersConfig config) {
    final Promise<RedisPubSubConnection> connectionFuture =
        client.getBootstrap().group().next().newPromise();
  ...
  public boolean freezeMaster(FreezeReason reason) {
    synchronized (this) {
      ...
      if (getFreezeReason() == null || getFreezeReason() == FreezeReason.RECONNECT) {
