...
/**
 * {@link ConnectionManager} for AWS ElastiCache Replication Groups. By providing all nodes of the
 * replication group to this manager, the role of each node can be polled to determine if a failover
 * has occurred resulting in a new master.
 *
      ...
      public void run() {
        try {
          ...
          for (URI addr : cfg.getNodeAddresses()) {
            ...
            } else if (Role.master.equals(role) && !master.equals(addr)
                && currentMaster.compareAndSet(master, addr)) {
