...
public class SentinelConnectionManager extends MasterSlaveConnectionManager {
  ...
  private final ConcurrentMap<String, RedisClient> sentinels = PlatformDependent
  ...
  public SentinelConnectionManager(SentinelServersConfig cfg, Config config) {
    ...
    for (URI addr : cfg.getSentinelAddresses()) {
      ...
      try {
        ...
        // TODO async
        List<String> master =
            connection.sync(RedisCommands.SENTINEL_GET_MASTER_ADDR_BY_NAME, cfg.getMasterName());
        ...
        List<Map<String, String>> sentinelSlaves =
            connection.sync(StringCodec.INSTANCE, RedisCommands.SENTINEL_SLAVES,
                cfg.getMasterName());
    ...
    List<Future<RedisPubSubConnection>> connectionFutures =
        new ArrayList<Future<RedisPubSubConnection>>(cfg.getSentinelAddresses().size());
    for (URI addr : cfg.getSentinelAddresses()) {
  ...
  private Future<RedisPubSubConnection> registerSentinel(final SentinelServersConfig cfg,
      final URI addr, final MasterSlaveServersConfig c) {
    RedisClient client = createClient(addr.getHost(), addr.getPort(), c.getConnectTimeout());
  ...
  protected void onSlaveAdded(URI addr, String msg) {
    ...
    if (parts.length > 4 && "slave".equals(parts[0])) {
      ...
      log.warn("onSlaveAdded. Invalid message: {} from Sentinel {}:{}", msg, addr.getHost(),
          addr.getPort());
    }
  }
  ...
  private void onNodeDown(URI sentinelAddr, String msg) {
    ...
    if (parts.length > 3) {
      ...
      log.warn("onSlaveDown. Invalid message: {} from Sentinel {}:{}", msg, sentinelAddr.getHost(),
          sentinelAddr.getPort());
    }
  }
  ...
  private void onNodeUp(URI addr, String msg) {
    ...
    if (parts.length > 3) {
      if ("slave".equals(parts[0])) {
        ...
        log.warn("onSlaveUp. Invalid message: {} from Sentinel {}:{}", msg, addr.getHost(),
            addr.getPort());
      }
    }
  }
