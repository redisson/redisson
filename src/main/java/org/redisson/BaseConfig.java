...
class BaseConfig<T extends BaseConfig<T>> {
  ...
  /**
   * Redis server will be excluded from the list of available nodes when sequential unsuccessful
   * execution attempts of any Redis command reaches <code>failedAttempts</code>.
   */
  ...
  /**
   * Password for Redis authentication. Should be null if not needed Default is <code>null</code>
   *
   * @param password
   */
   ...
   * <code>retryAttempts</code>. But if it sent succesfully then <code>timeout</code> will be
   * started.
   *
   * @see #timeout
   * @param retryAttempts
  ...
  /**
   * Reconnection attempt timeout to Redis server when it has been excluded from internal list of
   * available servers.
   *
   * On every such timeout event Redisson tries to connect to disconnected Redis server.
   *
  ...
  /**
   * Redis server will be excluded from the internal list of available nodes when sequential
   * unsuccessful execution attempts of any Redis command on this server reaches
   * <code>failedAttempts</code>.
   *
   * Default is 3
   *
   */
