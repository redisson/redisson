...
public class ClusterNodesDecoderTest {
    ...
    public ClusterNodesDecoderTest() throws IOException {
        ...
        String info = "7af253f8c20a3b3fbd481801bd361ec6643c6f0b 192.168.234.129:7001@17001 master - 0 1478865073260 8 connected 5461-10922\n" +
                ...
                "7945f4ed56b24172a8f1cd0eef4612ed93df049e :6379@16379 myself,master - 0 0 0 connected 0-16383\n";

        byte[] src = info.getBytes();
        buf.writeBytes(src);
        nodes = decoder.decode(buf, null);
    }
    ...
    @Test
    public void testImplicitHost() {
        ClusterNodeInfo node = nodes.get(6);
        Assert.assertEquals("127.0.0.1", node.getAddress().getHost());
        Assert.assertEquals(6379, node.getAddress().getPort());
    }
}
