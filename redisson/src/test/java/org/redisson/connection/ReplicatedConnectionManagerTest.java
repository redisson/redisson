...
public class ReplicatedConnectionManagerTest {
    ...
    public void cleanup() {
        if (manager != null) {
            for (RedisURI uri : new ArrayList<>(manager.getNodeConnections().keySet())) {
    ...
    public void testRemovesCachedConnectionOnTimeout() {
        ...
        manager.registerNodeConnection(uri, connection);
        ...
        Assertions.assertThat(manager.getNodeConnections()).isEmpty();
    ...
    public void testKeepsConnectionForNonTimeoutErrors() {
        ...
        Assertions.assertThat(manager.getNodeConnections()).containsEntry(uri, connection);
