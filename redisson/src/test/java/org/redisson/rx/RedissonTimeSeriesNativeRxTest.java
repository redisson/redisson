...
import org.redisson.api.RTimeSeriesNativeRx;
import org.redisson.api.RTimeSeriesNativesRx;
 ...
 *
 */
public class RedissonTimeSeriesNativeRxTest extends BaseRxTest {

    @Test
    public void testWritesAndReads() {
        RTimeSeriesNativeRx ts = redisson.getTimeSeriesNative("temperature");
    ...
    public void testEmptySeriesCompletesEmpty() {
        RTimeSeriesNativeRx ts = redisson.getTimeSeriesNative("empty");
        sync(ts.create());

        assertThat(ts.get().isEmpty().blockingGet()).isTrue();
        assertThat(ts.getLatest().isEmpty().blockingGet()).isTrue();
    }

    @Test
    public void testRules() {
        RTimeSeriesNativeRx source = redisson.getTimeSeriesNative("src");
        RTimeSeriesNativeRx destination = redisson.getTimeSeriesNative("dst");
        sync(source.create());
    ...
    public void testAdditions() {
        RTimeSeriesNativeRx ts = redisson.getTimeSeriesNative("extra");
    ...
    public void testFirstOnEmptySeriesCompletesEmpty() {
        RTimeSeriesNativeRx ts = redisson.getTimeSeriesNative("blank");
        sync(ts.create());

        assertThat(ts.first().isEmpty().blockingGet()).isTrue();
    }

    @Test
    public void testIteratorPagesLazily() {
        RTimeSeriesNativeRx ts = redisson.getTimeSeriesNative("walk");
        Map<Long, Double> samples = new LinkedHashMap<>();
        for (int i = 1; i <= 25; i++) {
        ...
        assertThat(ts.iterator(10).toList().blockingGet()).hasSize(25);
        assertThat(ts.iterator().map(TSSample::getTimestamp).toList().blockingGet())
                .isSorted().hasSize(25);

        assertThat(ts.iterator(2).take(3).map(TSSample::getValue).toList().blockingGet())
                .containsExactly(1.0, 2.0, 3.0);
    }

    @Test
    public void testAggregationValues() {
        RTimeSeriesNativeRx ts = redisson.getTimeSeriesNative("agg");
        sync(ts.add(1000, 10));
        sync(ts.add(1500, 20));
