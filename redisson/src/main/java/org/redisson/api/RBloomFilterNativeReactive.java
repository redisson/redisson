...
public interface RBloomFilterNativeReactive<T> extends RExpirableAsync {
     ...
     */
    Mono<List<Boolean>> add(Collection<T> elements);
     ...
     * @return list of booleans representing whether each element has been added successfully
     */
    Mono<List<Boolean>> insert(BloomFilterInsertOptions options, Collection<T> elements);
     ...
     */
    Mono<Void> init(BloomFilterInitOptions options);
     ...
     */
    Mono<List<Boolean>> exists(Collection<T> elements);
