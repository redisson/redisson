 ...
 * <p>
 * Unlike {@link RTimeSeriesAsync}, which stores arbitrary objects in a sorted set, this object is a
 ...
 * {@link RTimeSeriesNativesAsync} instead.
 * <p>
 * Requires <b>Redis 8.0.0 or higher, or the RedisTimeSeries module.</b>
 *
 * @author Nikita Koksharov
 */
public interface RTimeSeriesNativeAsync extends RExpirableAsync {
     ...
     */
    RFuture<Void> createAsync(TSCreateArgs args);
     ...
     */
    RFuture<Boolean> createIfAbsentAsync();
     ...
     */
    RFuture<Boolean> createIfAbsentAsync(TSCreateArgs args);
     ...
     */
    RFuture<Void> alterAsync(TSAlterArgs args);
     ...
     */
    RFuture<Long> addAsync(long timestamp, double value);
     ...
     */
    RFuture<Long> addCurrentAsync(double value);
     ...
     */
    RFuture<Long> addAsync(TSAddArgs args);
     ...
     */
    RFuture<List<Long>> addAllAsync(Map<Long, Double> samples);
     ...
     */
    RFuture<Long> incrementByAsync(double addend);
     ...
     * @return timestamp of the upserted sample
     */
    RFuture<Long> incrementByAsync(TSIncrArgs args);
     ...
     * @return timestamp of the upserted sample
     */
    RFuture<Long> decrementByAsync(double subtrahend);
     ...
     * @return timestamp of the upserted sample
     */
    RFuture<Long> decrementByAsync(TSIncrArgs args);
     ...
     */
    RFuture<Long> removeRangeAsync(long fromTimestamp, long toTimestamp);
     ...
     * @return last sample, or <code>null</code> if the series is empty
     */
    RFuture<TSSample> getLatestAsync();
     ...
     */
    RFuture<List<TSSample>> rangeAsync(long fromTimestamp, long toTimestamp);
     ...
     * @return samples ordered by increasing timestamp
     */
    RFuture<List<TSSample>> rangeAsync(TSRangeArgs args);
     ...
     */
    RFuture<List<TSSample>> rangeReversedAsync(long fromTimestamp, long toTimestamp);
     ...
     * @return samples ordered by decreasing timestamp
     */
    RFuture<List<TSSample>> rangeReversedAsync(TSRangeArgs args);
     ...
     * @return samples ordered by increasing timestamp
     */
    RFuture<List<TSSample>> readAsync(long timestamp);
     ...
     * @return samples ordered by increasing timestamp
     */
    RFuture<List<TSSample>> readAsync(TSReadArgs args);
     ...
     */
    RFuture<Void> createRuleAsync(TSRuleArgs args);
     ...
     */
    RFuture<Void> deleteRuleAsync(String destinationKey);

    /**
     * Returns every sample, oldest first, fetched ten at a time.
     *
     * @return asynchronous sample iterator
     */
    AsyncIterator<TSSample> iteratorAsync();
     ...
     * @return asynchronous sample iterator
     */
    AsyncIterator<TSSample> iteratorAsync(int count);
     ...
     * sample-returning method reports them. They are what {@link RTimeSeriesNativesAsync} filters
     * on, and its results carry them per series. Replace them with {@link #alterAsync(TSAlterArgs)}.
     * <p>
     * TS.INFO
     *
     * @return label names and values, empty if the series has none
     */
    RFuture<Map<String, String>> getLabelsAsync();
     ...
     */
    RFuture<TSInfo> getDebugInfoAsync();
