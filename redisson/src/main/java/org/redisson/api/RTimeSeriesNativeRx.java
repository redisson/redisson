 ...
 * <p>
 * Unlike {@link RTimeSeriesRx}, which stores arbitrary objects in a sorted set, this object is a
 ...
 * {@link RTimeSeriesNativesRx} instead.
 * <p>
 * Requires <b>Redis 8.0.0 or higher, or the RedisTimeSeries module.</b>
 *
 * @author Nikita Koksharov
 */
public interface RTimeSeriesNativeRx extends RExpirableRx {
     ...
     */
    Completable create(TSCreateArgs args);
     ...
     */
    Single<Boolean> createIfAbsent(TSCreateArgs args);
     ...
     */
    Completable alter(TSAlterArgs args);
     ...
     */
    Single<Long> add(long timestamp, double value);
     ...
     */
    Single<Long> addCurrent(double value);
     ...
     */
    Single<Long> add(TSAddArgs args);
     ...
     */
    Single<List<Long>> addAll(Map<Long, Double> samples);
     ...
     */
    Single<Long> incrementBy(double addend);
     ...
     * @return timestamp of the upserted sample
     */
    Single<Long> incrementBy(TSIncrArgs args);
     ...
     * @return timestamp of the upserted sample
     */
    Single<Long> decrementBy(double subtrahend);
     ...
     * @return timestamp of the upserted sample
     */
    Single<Long> decrementBy(TSIncrArgs args);

    /**
     * Removes every sample in the inclusive range.
     * <p>
     * Named apart from {@link RObject#delete()}, which removes the key itself.
     * <p>
     * TS.DEL
     *
     ...
     */
    Single<Long> removeRange(long fromTimestamp, long toTimestamp);
     ...
     */
    Single<List<TSSample>> range(long fromTimestamp, long toTimestamp);
     ...
     * @return samples ordered by increasing timestamp
     */
    Single<List<TSSample>> range(TSRangeArgs args);
     ...
     */
    Single<List<TSSample>> rangeReversed(long fromTimestamp, long toTimestamp);
     ...
     * @return samples ordered by decreasing timestamp
     */
    Single<List<TSSample>> rangeReversed(TSRangeArgs args);
     ...
     * @return samples ordered by increasing timestamp
     */
    Single<List<TSSample>> read(long timestamp);
     ...
     * @return samples ordered by increasing timestamp
     */
    Single<List<TSSample>> read(TSReadArgs args);
     ...
     */
    Completable createRule(TSRuleArgs args);
     ...
     */
    Completable deleteRule(String destinationKey);

    /**
     * Returns every sample, oldest first, fetched ten at a time.
     *
     * @return sample flowable
     */
    Flowable<TSSample> iterator();
     ...
     * @return sample flowable
     */
    Flowable<TSSample> iterator(int count);
     ...
     * sample-returning method reports them. They are what {@link RTimeSeriesNativesRx} filters on,
     * and its results carry them per series. Replace them with {@link #alter(TSAlterArgs)}.
     * <p>
     * TS.INFO
     *
     * @return label names and values, empty if the series has none
     */
    Single<Map<String, String>> getLabels();
