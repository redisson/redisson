...
public interface RLeasedMapRx<K, V> {
     ...
     * @return {@code true} if value has been stored, otherwise {@code false}
     */
    Single<Boolean> putWithLease(K key, V value, Duration ttl, long leaseToken);
     ...
     * @return {@code true} if value has been stored, otherwise {@code false}
     */
    Single<Boolean> putWithLease(K key, V value, Duration ttl, Duration maxIdleTime, long leaseToken);
}
