...
public final class RedissonRateLimiter extends RedissonExpirable implements RRateLimiter {
    ...
    public RFuture<Void> releaseAsync(long permits) {
        ...
        return commandExecutor.evalWriteAsync(getRawName(), StringCodec.INSTANCE, RedisCommands.EVAL_VOID,
                ...
                + "local entries = redis.call('zrange', permitsName, 0, -1, 'withscores');"
                + "for i = #entries - 1, 1, -2 do "
                + "    if toRelease <= 0 then break; end;"
                + "    local member = entries[i];"
                + "    local score = entries[i + 1];"
                + "    local random, p = struct.unpack('Bc0I', member);"
                + "    if p <= toRelease then "
                + "        redis.call('zrem', permitsName, member);"
                + "        toRelease = toRelease - p;"
                + "    else "
                + "        redis.call('zrem', permitsName, member);"
                + "        local left = p - toRelease;"
                + "        redis.call('zadd', permitsName, score, struct.pack('Bc0I', string.len(random), random, left));"
                + "        toRelease = 0;"
                + "    end;"
                + "end;"
