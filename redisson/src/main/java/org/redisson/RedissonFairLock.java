...
public class RedissonFairLock extends RedissonLock implements RLock {
    ...
    protected String isClientRegistered(){
        if (null != fairLockCache && fairLockCache.isLockRegistered(name)) {
            return "0";
        }
        return "1";
    }
    ...
    protected void unRegisterClient(){
        if (null != fairLockCache) {
            fairLockCache.unRegisterLock(name);
        }
    }
    ...
    <T> RFuture<T> tryLockInnerAsync(long waitTime, long leaseTime, TimeUnit unit, long threadId, RedisStrictCommand<T> command) {
        ...
        String isClientRegistered = isClientRegistered();
        String clientQueuePrefix = FairLockCache.getClientQueueName(name) + "*";
        String clientElementName = FairLockCache.getClientQueueElementName(name);
        long clientElementTtl = FairLockCache.getInterval();

        long currentTime = System.currentTimeMillis();
        if (command == RedisCommands.EVAL_NULL_BOOLEAN) {
            return getLock(threadId, command, wait, currentTime, isClientRegistered, clientQueuePrefix, clientElementName, clientElementTtl);
        }
    ...
    private <T> RFuture<T> getLock(long threadId, RedisStrictCommand<T> command, long wait, long currentTime, String isClientRegistered,
                                   String clientQueuePrefix, String clientElementName, long clientElementTtl) {
    ...
    private <T> RFuture<T> getLockTtl(long threadId, RedisStrictCommand<T> command, long wait, long currentTime, String isClientRegistered,
                                      String clientQueuePrefix, String clientElementName, long clientElementTtl) {
        RFuture<T> result = evalWriteAsync(getName(), LongCodec.INSTANCE, command,
                // remove unnecessary lock queue
                "if tonumber(ARGV[5]) == 0 then " +
    ...
    protected RFuture<Boolean> unlockInnerAsync(long threadId) {
        RFuture<Boolean> result =  evalWriteAsync(getName(), LongCodec.INSTANCE, RedisCommands.EVAL_BOOLEAN,
                // remove stale threads
                "while true do "
                + "local firstThreadId2 = redis.call('lindex', KEYS[2], 0);"
                + "if firstThreadId2 == false then "
