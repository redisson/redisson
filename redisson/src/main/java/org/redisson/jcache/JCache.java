...
public class JCache<K, V> extends RedissonObject implements Cache<K, V> {
    ...
    private static final RedisCommand<List<Object>> EVAL_GET_PUT = new RedisCommand<List<Object>>("EVAL", 
                                                                            new ObjectListReplayDecoder<Object>(), 11, ValueType.MAP, ValueType.OBJECT);
    ...
    final ConcurrentMap<CacheEntryListenerConfiguration<K, V>, Map<Integer, String>> listeners = 
    ...
    private boolean putValue(K key, Object value) {
        ...
        return (Boolean)get(commandExecutor.evalWriteAsync(getName(), codec, EVAL_PUT,
             ...
             creationTimeout, updateTimeout, System.currentTimeMillis(), key, value));
    }
        ...
        if (config.isWriteThrough()) {
            try {
                ...
                try {
                    ...
                    for (java.util.Map.Entry<K, V> deletedEntry : deletedKeys.entrySet()) {
    ...
    private boolean removeValue(K key) {
        return (Boolean)get(commandExecutor.evalWriteAsync(getName(), codec, EVAL_REMOVE_VALUE,
    ...
    private V getAndRemoveValue(K key) {
        return (V)get(commandExecutor.evalWriteAsync(getName(), codec, EVAL_GET_REMOVE_VALUE,
    ...
    protected Iterator<K> keyIterator() {
        return new RedissonBaseMapIterator<K, V, K>() {
            ...
            @Override
            protected V put(java.util.Map.Entry<ScanObjectEntry, ScanObjectEntry> entry, V value) {
                throw new UnsupportedOperationException();
            }
        };
    }
    ...
    private void registerCacheEntryListener(CacheEntryListenerConfiguration<K, V> cacheEntryListenerConfiguration, boolean addToConfig) {
        ...
        if (CacheEntryRemovedListener.class.isAssignableFrom(listener.getClass())) {
            RTopic<List<Object>> topic = redisson.getTopic(getRemovedChannelName(), new JCacheEventCodec(codec));
            ...
            values.put(listenerId, getRemovedChannelName());
        }
        if (CacheEntryCreatedListener.class.isAssignableFrom(listener.getClass())) {
            RTopic<List<Object>> topic = redisson.getTopic(getCreatedChannelName(), new JCacheEventCodec(codec));
            int listenerId = topic.addListener(new MessageListener<List<Object>>() {
                @Override
                public void onMessage(String channel, List<Object> msg) {
            ...
            values.put(listenerId, getCreatedChannelName());
        }
        if (CacheEntryUpdatedListener.class.isAssignableFrom(listener.getClass())) {
            RTopic<List<Object>> topic = redisson.getTopic(getUpdatedChannelName(), new JCacheEventCodec(codec));
            int listenerId = topic.addListener(new MessageListener<List<Object>>() {
                @Override
                public void onMessage(String channel, List<Object> msg) {
            ...
            values.put(listenerId, getUpdatedChannelName());
        }
        if (CacheEntryExpiredListener.class.isAssignableFrom(listener.getClass())) {
            RTopic<List<Object>> topic = redisson.getTopic(getExpiredChannelName(), new JCacheEventCodec(codec));
            int listenerId = topic.addListener(new MessageListener<List<Object>>() {
                @Override
                public void onMessage(String channel, List<Object> msg) {
            ...
            values.put(listenerId, getExpiredChannelName());
        }
    ...
    public void deregisterCacheEntryListener(CacheEntryListenerConfiguration<K, V> cacheEntryListenerConfiguration) {
        ...
        if (listenerIds != null) {
            for (java.util.Map.Entry<Integer, String> entry : listenerIds.entrySet()) {
    ...
    public Iterator<javax.cache.Cache.Entry<K, V>> iterator() {
        ...
        return new RedissonBaseMapIterator<K, V, javax.cache.Cache.Entry<K, V>>() {
            @Override
            protected javax.cache.Cache.Entry<K, V> getValue(java.util.Map.Entry<ScanObjectEntry, ScanObjectEntry> entry) {
