...
import io.quarkus.redisson.client.runtime.RedissonRuntimeConfig;
...
class QuarkusRedissonClientProcessor {
    ...
    void build(BuildProducer<SyntheticBeanBuildItem> beans,
                                  RedissonClientRecorder recorder,
                                  RedissonRuntimeConfig config
    ) throws IOException {
        ...
        if (stream != null) {
            ...
            final Supplier<RedissonClient> supplier = recorder.configureRedisson(new String(array, StandardCharsets.UTF_8));
            produceBean(beans, supplier);

        } else {
            final Supplier<RedissonClient> redissonClientSupplier = recorder.configureRedisson(config);
            produceBean(beans, redissonClientSupplier);
        }
    }

    private void produceBean(BuildProducer<SyntheticBeanBuildItem> beans, Supplier<RedissonClient> supplier) {
        SyntheticBeanBuildItem.ExtendedBeanConfigurator configurator = SyntheticBeanBuildItem
                .configure(RedissonClient.class)
                .addType(DotName.createSimple(Redisson.class.getName()))
